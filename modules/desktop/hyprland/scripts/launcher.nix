...
pkgs.writeShellScriptBin "launcher" ''
      ...
      wallpaper)
        ...
        r_override_display="entry{placeholder:'Select Display...';}listview{lines:${
          toString (builtins.length (opts.display or [ ]))
        };}"
  ...
  ${lib.concatMapStringsSep "\n" (
    ...
    "      ${toString (idx + 1)})\n        DISPLAY_OUTPUT=\"${display.output}\"\n        DISPLAY_ORIENTATION=\"${display.orientation}\"\n        ;;"
        ...
        swww img --outputs "$DISPLAY_OUTPUT" "$WALLPAPER_PATH" \
          --transition-type ${opts.wallpaper.transition.type or "center"} \
          --transition-step ${toString (opts.wallpaper.transition.step or 90)} \
          --transition-duration ${toString (opts.wallpaper.transition.duration or 1)} \
          --transition-fps ${toString (opts.wallpaper.transition.fps or 60)}
