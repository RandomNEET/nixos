...
pkgs.writeShellScriptBin "hypr-keybinds" ''
  ...
  TERMINAL_FM=${lib.escapeShellArg opts.terminalFileManager}
  ...
  KEYBINDERS=(
    ...
    "SUPER F" "Launch file manager" "$TERMINAL_FM"
    "SUPER E" "Launch editor" "$EDITOR"
    "SUPER B" "Launch browser" "$BROWSER"
    "CTRL ALT Delete" "Open system monitor" "$TERMINAL -e btop"
    ...
    ${
      if (config.home-manager.users.${opts.username}.programs.tmux.enable or false) then
