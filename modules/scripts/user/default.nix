...
let
  ...
  scriptDerivations =
    let
      ...
      _ = builtins.map (s: builtins.trace "Warning: ${s.error} (${toString s.file})" null) invalidScripts;

      validDerivations = builtins.map (s: s.drv) (builtins.filter (s: s.isValid) importedScripts);
    in
    validDerivations;
